...
import { getLocalDateKey, useRegisteredPhotos } from '../../context/RegisteredPhotosContext'
...
const DAILY_RANDOM_INSECT_LIMIT = 3
const DAILY_RANDOM_INSECT_STORAGE_PREFIX = 'little-biologist-random-insect-daily-count'
const MIN_RANDOM_SPAWN_DELAY_MS = 30 * 60 * 1000
const MAX_RANDOM_SPAWN_DELAY_MS = 6 * 60 * 60 * 1000
...
// 1시간 동안 재출몰하지 않는다.
function dailyRandomInsectKey(uid) {
  return `${DAILY_RANDOM_INSECT_STORAGE_PREFIX}:${uid ?? 'guest'}`
}

function readDailyRandomInsectCount(uid) {
  try {
    const today = getLocalDateKey()
    const record = JSON.parse(localStorage.getItem(dailyRandomInsectKey(uid)) || 'null')
    return record?.date === today && Number.isInteger(record.count) ? record.count : 0
  } catch {
    return 0
  }
}

function recordDailyRandomInsectAppearance(uid) {
  const count = readDailyRandomInsectCount(uid)
  if (count >= DAILY_RANDOM_INSECT_LIMIT) return false
  localStorage.setItem(dailyRandomInsectKey(uid), JSON.stringify({ date: getLocalDateKey(), count: count + 1 }))
  return true
}
...
function getRandomSpawnDelay() {
  ...
  const remainingTodayMs = Math.max(0, endOfDay.getTime() - now.getTime())
  if (remainingTodayMs <= MIN_RANDOM_SPAWN_DELAY_MS) return Math.max(1000, remainingTodayMs)
  const maxDelay = Math.min(MAX_RANDOM_SPAWN_DELAY_MS, remainingTodayMs)
  return MIN_RANDOM_SPAWN_DELAY_MS + Math.random() * (maxDelay - MIN_RANDOM_SPAWN_DELAY_MS)
}
...
export default function RandomInsectEffect() {
  ...
  const hasTriedInitialSpawnRef = useRef(false)
  ...
  const showRandomInsect = useCallback(() => {
    if (unregisteredPool.length === 0) return
    if (!recordDailyRandomInsectAppearance(user?.uid)) return
    setPoint(randomPoint())
    setIsVisible(true)
  }, [unregisteredPool.length, user?.uid])

  const scheduleRandomInsect = useCallback(() => {
    window.clearTimeout(respawnTimerRef.current)
    if (unregisteredPool.length === 0) return
    if (readDailyRandomInsectCount(user?.uid) >= DAILY_RANDOM_INSECT_LIMIT) return
    respawnTimerRef.current = window.setTimeout(showRandomInsect, getRandomSpawnDelay())
  }, [showRandomInsect, unregisteredPool.length, user?.uid])
  ...
  useEffect(() => {
    if (isTutorialStep || hasTriedInitialSpawnRef.current) return
    hasTriedInitialSpawnRef.current = true
    scheduleRandomInsect()
  }, [isTutorialStep, scheduleRandomInsect])
